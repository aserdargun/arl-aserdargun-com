...
Additional controlled scenarios exercise temporary failures, persistent timeouts, incorrect calculation, missing evidence, context overflow and budget exhaustion. The twelve-chapter **Agent Runtime 101** guide introduces each boundary. Mobile and reduced-motion sessions start with a lightweight text topology; 3D loads on request. The mobile inspector opens as a bottom sheet. Opening a review, document, lesson or export pauses playback. A failed 3D load retains the usable text topology.
...
`release.json` records the Git SHA, whether the source checkout has uncommitted changes, and SHA-256 hashes of all deployed artifact files. `npm run verify:artifact` checks this manifest against the local build. Production releases are verified against GitHub Actions, Azure environment readiness, live manifest/asset hashes and browser behavior. Custom-domain and DNS configuration are not part of this workflow.
