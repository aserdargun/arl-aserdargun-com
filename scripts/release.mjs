...
if (process.env.GITHUB_ACTIONS === "true")
  assert.equal(sourceDirty, false, "CI release requires a clean checkout");
...
if (process.argv[2] === "write") {
  await writeFile(
    ...
    JSON.stringify(
      {
        schemaVersion: 1,
        repository: "aserdargun/arl-aserdargun-com",
        gitSha: sha,
        sourceDirty,
...
assert.equal(release.sourceDirty, sourceDirty);
...
assert(
  ...
  "Missing lazy 3D bundle",
);
console.log(
  `Verified ${files.length} artifact files for ${sha}${sourceDirty ? " (uncommitted local changes)" : ""}`,
);
